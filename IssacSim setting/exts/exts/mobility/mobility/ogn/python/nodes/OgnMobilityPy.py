...
class OgnMobilityPyInternalState:
    ...
    def __init__(self):
        ...
        self.status = False
        self.prim_path = "/World/transporter"
        ...
        self.object=XFormPrim("/World/purpose_position1")
        self.position,_=self.object.get_world_pose()

        self.target_transporter_position = self.position
        self.target_transporter_orientation = numpy_utils.euler_angles_to_quats(np.array([0.0,0.0,np.pi/2]))
    ...
    def move_cw(self):
        while True:
            ...
            if abs(angle_diff) > 0.03:
                ...
                control_actions = ArticulationActions(joint_velocities=velocity)
                self.state_time += 1
                if self.state_time >= 30:  # 60 프레임 = 약 1초
    ...
    def move_forward(self,distance,axis):
        ...
        while True:
            ...
            target_position = distance - self.transporter_position[i]
...
class OgnMobilityPy:
    ...
    def internal_state():
        ...
        return OgnMobilityPyInternalState()
        ...
        try:
            ...
            if state.input_state == 1:
                if state.state == "move_forward_x":
                    ...
                    state.transporter._articulation_view.apply_action(control_actions)
                elif state.state == "move_forward_y":
